...
# piroplasms
# WBC
# RBC, Hgb, PCV% (all three optional, not used in the calculations here)
# exitDay, exitType
#
# optional column: group (can be set from a csv input by another routine)
...
########################################################
###### ANCILLARY FUNCTIONS (some of them unused) #######
########################################################

# Mean of values greater than 0 with this ancillary function.
...
# The below two functions are no longer useful
# if all numerical variables (except temp)
...
# accumulate from the first non-zero value.
...
# The below is used to calculate the log of the maximum
# piroplasm severity so far.
...
# fever average: does the mean of the signed deviance values from 39.4 °C,
# from the first position where there IS fever (temp > 39.4)
...
# The following function adds a column "group" to a tibble,
# which are meant to be experimental groups taken from a tibble
# with only two columns: (1) animalID and (2) group. For now,
# the flexibility comes from the fact we ignore the order of columns in the input.
# We could as well ignore the column names and rely on their order.
# NB/WARNING: for now, this is just for a single experiment.
add_or_overwrite_groups <- function(main_tibble, grouping_tibble) {
  # need to be a bit careful in case main_tibble **already** has a `group` column
  if ("group" %in% names(main_tibble))
    main_tibble %<>% select(-group) 
  
  # then we do a normal join() on animal IDs
  main_tibble %>% left_join(grouping_tibble, by = "animalID") -> res
  res %<>% relocate(group, .after = animalID)
...
########################################################
## FUNCTIONS CALCULATING THE ECF INDEX ITSELF ##########
########################################################

# The following functions are meant to be used in the context of a call to
# mutate() they take as input equally-sized numerical vectors and return
...
############################################################
## MAIN TRANSFORM FUNCTION ADDING ECF INDICES TO A TIBBLE ##
############################################################

# tibble in, tibble out:
add_ECF_RI <- function(tibb) {
  
  # for later on, we sort the observations by:
  # (1) Experiment name
  # (2) Animal id
  # (3) Day post challenge
  # and we remove potential obseravtions with dayPostChallenge == 0
  tibb %<>% filter(dayPostChallenge > 0) %>%
    arrange(experimentName, animalID, dayPostChallenge)
  # note the above could well have already be done upstream,
  ...
  tibb %>% ungroup() %>% mutate(experimentDuration = max(dayPostChallenge),
  ...
  # and finally we add the columns with the calculated Reaction Indices
  sampledat %<>% mutate(
...
### TEST ###
# add_ECF_RI(tmp) -> computed
# grp <- read_csv("IACUC2023-26_groups.csv")
# add_groups(computed, grp) -> computed
# write_csv(computed, file = "computed.csv")
